...
#             This is essential for understanding why a feature is influential.
#
...
#         → steer_feature() × 3         (top recurring features)
#         → get_subgraph_node_ids_for_feature() × 3
#         → label_nodes_batch() × 3
#         → save_circuit()
#
# 0d. CONFIRM YOUR UNDERSTANDING.
...
5. ── ACROSS-GRAPHS ANALYSIS ──────────────────────────────────────────────────
   ...
   Print the full merged table, then:

   a. Call get_edges_for_node(G, node_id) on the top 2 recurring features by
      avg_influence (use any graph in which they appear). Print the incoming
      and outgoing edges. This reveals the immediate circuit neighbourhood —
      what feeds into these features and what they feed into.
   ...
   b. Identify which layer appears most frequently among the top 15 recurring
      ...
      loaded graph). Print all nodes at that layer ranked by influence. This
      gives the full picture of what that layer is doing across the circuit.
...
6. ── PER-PROMPT INTERPRETATION ───────────────────────────────────────────────
   ...
   anything extra:
     a. Labels 40 nodes (top_n_nodes=40) grouped into early / middle / late
        bands, printing them as they arrive band by band.
     ...
     d. Saves a PNG figure to graphs/{slug}__{token}_causal_paths.png showing
        the paths as a layered diagram with edge widths proportional to weight.
   ...
   For each prompt your narrative must answer:
     ...
     4. What was the dominant excitatory path (name the nodes and edge weights)?
...
9. ── COMPLETE THE PAPER ──────────────────────────────────────────────────────
   ...
   - Abstract: fill in with actual numbers (prompts run, graphs generated,
     recurring features found at 50% threshold, top feature labels, dominant
     causal paths identified, steering results summary)
